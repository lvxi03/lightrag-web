<template>
  <div class="page">
    <canvas ref="canvas" class="canvas-particle"></canvas>
      ...
      <transition name="el-fade-in-linear" mode="out-in">
        <div style="text-align: center;margin: 0 20px;height: 100%" v-if="active === 0">
          ...
          <div style="margin-top: 70px">
            ...
            <el-button type="primary" @click="goToHome" plain style="margin-left: 20px;">返回登录页面</el-button>
          </div>
        </div>
      </transition>
      <transition name="el-fade-in-linear" mode="out-in">
...
const validateEmail = () => {
  coldTime.value = 60;
  get(`/ask-code/?email=${form.email}&type=reset`, () => {
...
const confirmReset = () => {
  formRef.value.validate((isValid) => {
    if (isValid) {
      // 验证通过，跳转到 reset_password 页面，并传递邮箱地址
      router.push({ name: 'reset_password', query: { email: form.email } });
    } else {
      ElMessage.error('请填写正确的电子邮件和验证码');
    }
  });
};
...
const doReset = () => {
  formRef.value.validate((isValid) => {
    if (isValid) {
      ...
      console.log('Sending data:', requestData);  // 打印发送的数据
